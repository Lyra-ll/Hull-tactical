...
def train_fold_ae(ae_params, X_train_raw, y_train_raw, sw_train_raw, X_val_raw, y_val_raw, seeds=[42, 2024, 777]):
    """
    [升级 V1.2] 在当前CV折内部，使用多个随机种子训练，并返回所有训练好的模型列表。
    """
    ...
    mean = X_train_filled.mean(axis=0).values
    std = X_train_filled.std(axis=0).values
    std[std == 0] = 1.0
    ...
    for seed in seeds:
        ...
        train_loader = DataLoader(
            ...
            batch_size=512, 
            shuffle=True,
            num_workers=8,  # <-- [优化1] 开启4个子进程在后台准备数据
            pin_memory=True # <-- [优化2] 将数据预先锁在内存中，加速传输
        )
        ...
        if 'noise_std' in ae_params: model_architecture_params['noise_std'] = ae_params['noise_std']
        ...
        for epoch in range(epochs):
            ...
            with torch.no_grad():
                ...
                y_val_tensor = torch.tensor(y_val_raw.values, dtype=torch.float32).unsqueeze(1).to(device)
