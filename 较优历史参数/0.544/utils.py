    ...
    for seed in seeds:
        ...
        for epoch in range(epochs):
            ...
            for features, mask, target, weights in train_loader:
                features, mask, target, weights = features.to(device), mask.to(device), target.to(device), weights.to(device)
            ...
            model.eval()
            with torch.no_grad():
                val_tensor = torch.tensor(X_val_scaled_np, dtype=torch.float32).to(device)
                y_val_tensor = torch.tensor(y_val_np, dtype=torch.float32).to(device)
                sw_val_tensor = torch.tensor(sw_val_np, dtype=torch.float32).unsqueeze(1).to(device)
                val_mask_tensor = torch.tensor(X_val_mask_np, dtype=torch.bool).to(device)
