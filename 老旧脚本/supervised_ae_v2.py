# =================================================================
# supervised_ae_v2_causal.py (V1.0 - Causal Feature Engineering)
# =================================================================
import pandas as pd
import numpy as np
import torch
...
import copy
import time
from sklearn.model_selection import TimeSeriesSplit # <--- 我们需要的唯一分割器
...
print(f"--- 步骤2：Purged & Embargoed 时序交叉验证已准备就绪 ---")

# ======================== 核心修正点 ========================
for fold, (train_indices, val_indices) in enumerate(tscv.split(features_df)): # <--- 使用 tscv 而不是 kf
# ==========================================================
    ...
    train_target = torch.tensor(y_train_raw.fillna(0).values, dtype=torch.float32).unsqueeze(1)
    
    val_mask = torch.tensor(X_val_raw.isnull().values, dtype=torch.bool)
    val_features = torch.tensor(X_val_scaled, dtype=torch.float32)
    val_target = torch.tensor(y_val_raw.fillna(0).values, dtype=torch.float32).unsqueeze(1)
...
output_filename = 'train_v7_causal_ae_features.csv'
final_df.to_csv(output_filename, index=False)

print(f"\n✅ 我们的“绝对诚实”的因果性武器已锻造完毕！")
