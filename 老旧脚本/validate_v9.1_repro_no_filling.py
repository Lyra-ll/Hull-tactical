...
# =================================================================
# validate_v9.1_repro_no_filling.py (不填充复现版)
# 目的: 在“不填充”策略下，严格复现“巅峰对决”，
#       以检验我们 0.529 AUC 成绩的真实来源。
# =================================================================
...
# ================= 2. 核心函数 (已升级) =================
# --- 升级：现在使用“不填充”函数 ---
def preprocess_data_no_filling(X_train, X_val):
    """根据最终战略，只返回带NaN的原始数据"""
...
def run_validation(X, y, sample_weight, params, group_name):
    ...
    for fold, (train_idx, val_idx) in enumerate(tscv.split(X)):
        ...
        # 确保使用与v9一致的填充策略
        X_train_processed, X_val_processed = preprocess_data_no_filling(X_train_raw, X_val_raw)
        
        model = lgb.LGBMClassifier(**params)
        model.fit(X_train_processed, y_train, sample_weight=sw_train,
                  eval_set=[(X_val_processed, y_val)], callbacks=[lgb.early_stopping(100, verbose=False)])
