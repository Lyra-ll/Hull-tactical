...
def train_fold_ae(ae_params, X_train_scaled_np, y_train_np, sw_train_np, X_val_scaled_np, y_val_np, sw_val_np, train_mask_np, X_val_mask_np, seeds=[42, 2024]):
    ...
    print(f"    > [AE] 开始训练，样本数: {len(train_dataset)}, Seeds: {len(seeds)}")
    print(f"    > [AE] 设备: {device} ({'GPU' if device.type == 'cuda' else 'CPU'})")
    if device.type == 'cuda':
        print(f"    > [AE] GPU: {torch.cuda.get_device_name(torch.cuda.current_device())}")
        print(f"    > [AE] GPU内存: {torch.cuda.get_device_properties(0).total_memory / 1024**3:.1f}GB")
        print(f"    > [AE] CUDA版本: {torch.version.cuda}")
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        for epoch in range(epochs):
            ...
            print(f"      > Epoch {epoch+1}/{epochs}, Val_loss: {val_loss:.6f}, Time: {epoch_time:.2f}s")
        ...
        print(f"    > [AE] Seed {seed_idx+1} 完成，耗时: {seed_time:.2f}秒")
    
    total_time = time.time() - fold_start_time
    print(f"    > [AE] Fold训练完成，总耗时: {total_time:.2f}秒")
